...
npx dotenv -e .env -- npx tsx scripts/setup-db.ts # 初始化数据库
```
...
- 图片消息 → 直接 fetch Vision 模型 API (跳过 Agent) → SSE 转 AI SDK data stream
- 两条路径共用 `POST /api/chat`，通过 `hasImages` 分支

### 工具系统
- `knowledge_search`: RAG 知识库检索 (Supabase pgvector)
- `tavily_search`: 联网实时搜索
- Agent 优先查知识库，无结果再联网

### 数据隔离
- 所有数据通过 `user_id` 隔离
- API 路由统一用 `getCurrentUser()` (`lib/supabase-server.ts`) 鉴权
...
- 组件内部的子组件（如 `UserMessageContent`）可以定义在同一文件中，不强制拆分

### 样式
- 使用 Tailwind CSS 原子类，不写自定义 CSS（`globals.css` 除外）
- 深色主题，主色调：背景 `#1e1e19`、文字 `#ececec` / `#e7e7e4`、强调色 `#d48c66`
- 透明度边框用 `border-white/5`、`border-white/10`、`border-white/20` 层级
- 需要合并 className 时使用 `clsx()` 或直接模板字符串，项目已安装 `tailwind-merge` 和 `clsx`

### API 路由
- 每个路由必须在开头调用 `getCurrentUser()` 鉴权，未登录返回 `401`
...
- 查询用户数据时必须带 `where: { user_id: user.id }` 过滤
